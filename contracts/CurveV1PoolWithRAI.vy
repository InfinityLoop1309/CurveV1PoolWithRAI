...
@dev Swaps between 1DollarCryptos and RAI
"""
...
def _xp() -> uint256[N_COINS]:
    result: uint256[N_COINS] = [self._get_scaled_redemption_price(), 10**18]
    for i in range(N_COINS):
...
def calc_token_amount(_amounts: uint256[N_COINS], _is_deposit: bool) -> uint256:
    ...
    """
    amp: uint256 = self._A()
    rates: uint256[N_COINS] = [self._get_scaled_redemption_price(), 10**18]
...
def add_liquidity(_amounts: uint256[N_COINS], _min_mint_amount: uint256) -> uint256:
    ...
    """
    assert not self.is_killed ,"killed" # dev: is killed
    amp: uint256 = self._A()
    rates: uint256[N_COINS] = [self._get_scaled_redemption_price(), 10**18]
    ...
    for i in range(N_COINS):
        if _amounts[i] > 0:
            ...
            if len(response) > 0:
                assert convert(response, bool),"transfer failed"  # dev: failed transfer
